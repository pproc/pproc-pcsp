...
public class CrawlerMethods {
    ...
    private static String getNextPagePetition(Document doc) {
        ...
        try {
            ...
            Log.debug(CrawlerMethods.class, "[getNextPagePetition] nextPetition pages %s - %s", pag[0], pag[1]);
        } catch (java.lang.ArrayIndexOutOfBoundsException ex) {
            Log.error(CrawlerMethods.class, "[getNextPagePetition] ArrayIndexOutOfBoundsException");
            return null;
        }
        return petition;
    }
    ...
    private static Set<String> getIds(Document doc) {
        ...
        if (matches.length < 20) {
            ...
            Log.warning(CrawlerMethods.class, "MIRA DOC.HTML PARA CONFIRMAR IDS PLATAFORMA");
        }
        return ids;
    }
    ...
    private static Document retrieveDocument(String petition, String sessionIdCookie) {
        ...
        while (!success && tryCount < Utils.MAX_TRY_COUNT) {
            try {
                ...
                Log.warning(CrawlerMethods.class, "[retrieveDocument] getNextDocument try number %d failed", tryCount);
            } catch (Exception e) {
                Log.error(CrawlerMethods.class, "[retrieveDocument] getNextDocument unexpected error");
                e.printStackTrace();
            }
        }
        if (!success)
            Log.error(CrawlerMethods.class, "getNextDocument could not retrieve data");
        return document;
    }
    ...
    public static void getXmlLinks(Database database) throws IOException {
        ...
        Log.info(CrawlerMethods.class, "[getXmlLinks] get to main web to get sessionIdCookie successful");
        ...
        Log.info(CrawlerMethods.class, "[getXmlLinks] get to advanced search to get paged petition base url successful");
        ...
        while (actualPage(document) < finalPage(document) && numberOfErrors < 3) {
            if (actualPage(document) != -1 && finalPage(document) != -1)
                ...
                Log.error(CrawlerMethods.class,
                        "[getXmlLinks] web document incorrect, trying same last petition again (%d)", ++numberOfErrors);
            ...
            Log.info(CrawlerMethods.class, "[getXmlLinks] page %d/%d done - %.4f%%", actualPage(document),
                    finalPage(document), por);
        }
    }
