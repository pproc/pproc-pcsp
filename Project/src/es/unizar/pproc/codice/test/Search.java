...
public class Search {
    ...
    public static void parseXmlsIf(Database database) throws FileNotFoundException, UnsupportedEncodingException,
        ...
        while (!(xmlLinks = database.selectByLimit(true, 2000, Database.FLAG_CHECKED_PARSED)).isEmpty()) {
                ...
                try {
                    ...
                    if (altString.length() < 4) {
                        ...
                        try {
                            ...
                            Log.error(Methods.class, "[parseXmls] error parsing xml %d, see debug/codice_doc.xml",
                                    xmlLink.id);
                            Utils.writeInfile("debug/codice_doc.xml", document.asXML());
                            ex.printStackTrace();
                            return;
                        }
                        Log.debug(Methods.class, "[parseXmls] xml %d parsed and inserted in model", xmlLink.id);
                        tempXmlLinks.add(xmlLink);
                    }
                } catch (DocumentException exc) {
                    Log.error(Methods.class, "[parseXmls] error reading xml %d [%d bytes]", xmlLink.id,
            ...
            Log.info(Methods.class, "[parseXmls] %d/%d parsed documents (%d errors so far) (noPlatformIDCount = "
