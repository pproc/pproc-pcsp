...
public class Codice2Pproc {
	...
	private static void parseProcurementProjectElement(Model model, Element altElement, Resource rootResource) {
		...
		if (altElement.element("RequestForTenderLine") != null) {
			...
			for (Iterator<?> iter = altElement.elementIterator("RequestForTenderLine"); iter.hasNext();) {
				...
				if ((altString = altElement2.elementText("ID")) != null)
					offeringResource = model.createResource(objectResource + "/Offering_" + altString);
				else
					offeringResource = model.createResource(objectResource + "/Offering_" + offeringId++);
		...
		if (altElement.element("RealizedLocation") != null) {
			...
			objectResource.addProperty(PPROC.location, placeResource);
			...
			if ((altElement2 = altElement.element("RealizedLocation").element("Address")) != null) {
				...
				if (altElement2.element("Country") != null
						&& (altString = altElement2.element("Country").elementText("IdentificationCode")) != null)
		...
		if (altElement.element("ContractExtension") != null) {
			...
			if (altElement.element("ContractExtension").element("OptionValidityPeriod") != null
					&& (altString = altElement.element("ContractExtension").element("OptionValidityPeriod")
	...
	private static void parseTenderingTermsElement(Model model, Element altElement, Resource rootResource,
		...
		if (altElement.element("TenderRecipientParty") != null) {
			...
			if (altElement.element("TenderRecipientParty").element("PartyName") != null
					&& (altString = altElement.element("TenderRecipientParty").element("PartyName").elementText("Name")) != null)
				placeResource.addProperty(S.name, altString, XSDDatatype.XSDstring);

			// TenderingTerms/TenderRecipientParty/PostalAddress
			if (altElement.element("TenderRecipientParty").element("PostalAddress") != null) {
				Resource postalAddressResource = model.createResource(placeResource + "/PostalAddress");
				postalAddressResource.addProperty(RDF.type, S.PostalAddress);
				placeResource.addProperty(S.address, postalAddressResource);
	...
	public static void parseCodiceXML(Model model, Document document) throws Exception {
		...
		if ((altElement = root.element("ContractingParty")) != null
				|| (altElement = root.element("ContractingAuthorityParty")) != null) {
			altString = null;
			altString2 = null;
			for (Iterator<?> iter = altElement.element("Party").elementIterator("PartyIdentification"); iter.hasNext();) {
				...
				else if (altElement2.element("ID").attributeValue("schemeName").equals("CIF"))
					altString2 = altElement2.elementText("ID");
			}
		...
		if ((altElement = root.element("TenderingProcess")) != null) {
			...
			if ((altString = altElement.elementText("ProcedureCode")) != null)
				...
				case "5":
					...
					break;
				case "100":
					// "Normas internas", not defined in PPROC
				}
			...
			if (altElement.element("FrameworkAgreement") != null && model.containsResource(PPROC.ConclusionContract)) {
				...
				// TODO add dcterms:description from TenderingProcess/FrameworkAgreement//Justification
			}
			...
			// TenderingProcess/AdditionalDocumentReference
			if (altElement.element("AdditionalDocumentReference") != null
					&& altElement.element("AdditionalDocumentReference").element("Attachment") != null
					&& (altString = altElement.element("AdditionalDocumentReference").element("Attachment")
				...
				// TenderingProcess/AdditionalDocumentReference/IssueDate
				if ((altString = altElement.element("AdditionalDocumentReference").elementText("IssueDate")) != null)
					documentResource.addProperty(DCTerms.dateSubmitted, altString);

			}

			// TODO TenderingProcess/NoticeDocumentReference?
		...
		if (root.element("TenderResult") != null) {
			...
			for (Iterator<?> iter = root.elementIterator("TenderResult"); iter.hasNext();) {
				...
				if (isAwarded || isFormalized) {
					...
					// TODO add pproc:formalizedDate property?
					if ((altString = altElement.elementText("AwardDate")) != null) {
						tenderResource.addProperty(PPROC.awardDate, altString, XSDDatatype.XSDdate);
					}
					...
					if ((altElement2 = altElement.element("WinningParty")) != null) {
						...
						if (altString != null)
							altString = BASE_URI_ORGANIZATION + altString.replace(" ", "");
						else
		...
		if (root.element("TechnicalDocumentReference") != null
			...
			// TechnicalDocumentReference/IssueDate
